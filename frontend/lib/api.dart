...
class EcoClickAPI {
  ...
  static Future<List<dynamic>> getUsers() async {
    ...
    if (r.statusCode >= 400) throw Exception('Error ${r.statusCode}: ${r.body}');
  ...
  static Future<List<Map<String, dynamic>>> getUserAchievements(int userId) async {
    final r = await http.get(Uri.parse('$baseUrl/userAchievements?userId=$userId'));
    if (r.statusCode >= 400) throw Exception('Error ${r.statusCode}: ${r.body}');
  ...
  static Future<Map<String, dynamic>> postProgress({
    ...
    final r = await http.post(
      ...
      body: jsonEncode({'userId': userId, 'achievementId': achievementId, 'date': date}),
    );
    if (r.statusCode == 409) {
